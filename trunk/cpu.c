...
void compute_data_gradient(chi2_info * data_info, float * image, float * data_gradient)
{
	...
	for (ii = 0; ii < image_width; ii++)
	{
		for (jj = 0; jj < image_width; jj++)
		{
			...
			for (kk = 0; kk < npow; kk++)
			{
				data_gradient[ii + jj * image_width] += 4.0 * data_err[kk] * data_err[kk] * invflux * (mock[kk]
						- data[kk]) * creal(conj(visi[kk]) * (DFT_tablex[image_width * kk + ii]
						* DFT_tabley[image_width * kk + jj] - visi[kk]));
			}
			...
			for (kk = 0; kk < nbis; kk++)
			{
				...
				vabder = DFT_tablex[oifits_info.bsref[kk].ab.uvpnt * image_width + ii]
						* DFT_tabley[oifits_info.bsref[kk].ab.uvpnt * image_width + jj];
				vbcder = DFT_tablex[oifits_info.bsref[kk].bc.uvpnt * image_width + ii]
						* DFT_tabley[oifits_info.bsref[kk].bc.uvpnt * image_width + jj];
				vcader = DFT_tablex[oifits_info.bsref[kk].ca.uvpnt * image_width + ii]
						* DFT_tabley[oifits_info.bsref[kk].ca.uvpnt * image_width + jj];

				if (oifits_info.bsref[kk].ab.sign < 0)
				{
					vab = conj(vab);
				}
				if (oifits_info.bsref[kk].bc.sign < 0)
				{
					vbc = conj(vbc);
				}
...
float GullSkilling_entropy_diff(int image_width, int x_old, int y_old, int x_new, int y_new, float old_flux,
		float new_flux, float * default_model)
{
...
float image2chi2(chi2_info * info, float * image)
{
	...
	vis2data(i2v_info.npow, i2v_info.nbis, i2v_info.oifits_info, i2v_info.data_phasor, i2v_info.dft_y, i2v_info.dft_y,
			i2v_info.visi, i2v_info.mock);
...
float L2_diff(int image_width, int x_old, int y_old, int x_new, int y_new, float * image, float * default_model)
{
	int position_old = x_old + y_old * image_width;
	int position_new = x_new + y_new * image_width;
		...
		float * default_model)
{
	...
	case 4:
	{
		...
		for (i = 0; i < image_width; i++)
		{
			for (j = 0; j < image_width; j++)
			{
				default_model[j * image_width + i] = sigma / (square(sigma) + square((float) image_width / 2 - i)
						+ square((float) image_width / 2 - j));
				flux += default_model[j * image_width + i];
			}
		}
